...
| 1 | `low` | Background noise or light probing. | `1s` / `1.05` / `120s` | `0.75` / `0.60` | `0.25s ×1.35`, `30s` | `0.02`, `30s`, `0.75`, `5` | `0.4 / 0.7 / 0.3`, `2`, `1.25` | `900s` |
| 2 | `medium` | Balanced; good default for mixed networks. | `1s` / `1.10` / `300s` | `0.60` / `0.50` | `0.5s ×1.5`, `45s` | `0.05`, `60s`, `0.60`, `3` | `0.9 / 1.3 / 0.8`, `2`, `1.5` | `600s` |
| 3 | `high` | Sustained spam runs with light collateral. | `2s` / `1.15` / `300s` | `0.50` / `0.45` | `1.5s ×2.5`, `60s` | `0.10`, `90s`, `0.50`, `2` | `1.1 / 1.7 / 1.1`, `2`, `1.7` | `450s` |
...
The interaction weights multiply the base delay when multiple heuristics fire (e.g., `low-entropy + spammy-kmer`). Level 2 keeps the base delay at one second for single-signal hits but applies a ~2–3× boost when two or more signals stack, producing the desired “J-curve” without punishing regular users. Fan-out penalties at that level now start at only half a second (×1.5 ramp) so fast-but-legitimate chatters merely see a short pause, while true Sybil bursts still add hefty queue time once other signals land. The new trust decay parameters (`trust_decay`, `trust_weight`) further nudge the multiplier for short-lived connections: brand-new clients pay the full `trust_weight`, but once they have been around longer than `trust_decay` seconds the trust factor drops back to 1.0.
